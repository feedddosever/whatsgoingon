...
## The Florida overclaim, found 2026-09-20

An external research pass audited `data/fl/acceptance-rules.ts` row by row
against the **August 2026** edition of the ACC credit-by-exam equivalencies, and
found the file wrong in the way this project exists to prevent.

The first version assumed that any award on Florida's statewide table cleared a
general-education core area. **It does not.** The table awards credit for a
*named course*, and only some of those courses carry the `core` tag. Eleven rows
claimed a core area they do not get:

> AP Microeconomics · AP Human Geography · AP Comparative Government ·
> AP Spanish · AP European History · CLEP Introductory Sociology ·
> CLEP Humanities · CLEP American Literature · AP US History (at a score of 3)

A student reading those would have believed a requirement was handled, been
awarded the credit, and still owed the requirement — with nothing on screen
looking wrong. All are now `satisfies_areas: []`, which produces the same
`credit_not_toward_ge` warning as CLEP at a CSU.

Also corrected:

- **AP English Literature was on the wrong area.** The only core course it can
  produce is ENC X101, which is *Communication*, not Humanities — and it is the
  same course AP Language awards, so the two are an either/or.
- **The AP sciences and Calculus need a 4, not a 3**, and award **4 credits**.
- **CLEP Natural Sciences has no row at all** — "no direct equivalent". The rule
  is deleted, so the engine correctly says it has no record rather than
  inventing elective credit.
- **Two course rows came off the core** for 2024-25: `fl-syg-2000` (SYG X000)
  and `fl-mgf-1106` (MGF X106).
...
`data/research/` holds the source: a five-pass consolidation covering all 51
jurisdictions with governing citations and a per-row research grade. All 51 now
carry a finding, in three kinds rather than two:

| | States | Meaning |
|---|---|---|
| `yes` | 46 | A statewide instrument exists and we can state it |
| `none` | 5 | We looked; the state genuinely has none |
| `unknown` | 0 | We have not looked |

"There is no statewide rule" is a finding a student can act on. It is not the
same as "we have not checked", and the picker no longer headlines the first as
THE STATEWIDE RULE.

Corrections to rows already shipped: Georgia was **Core IMPACTS**, not "Areas
A–F"; Wisconsin's UCTA is **72 credits**, not 30; Arizona is moving to a
**Reimagined AGEC** for 2026-27; SUNY's framework was amended by **Res. 2024-64**
for students entering Fall 2026; South Carolina's "86 transferable courses" is
**31** by the CHE's own audit.

**Oregon and Washington count in QUARTER credits**, and now say so in the row.
Every figure this engine computes assumes semester units, so a quarter state
priced against that assumption is wrong by a factor of 1.5 with nothing to
complain — the highest-risk gap in the project, and it was live in Oregon's row.

Each row carries its governing **authority** (statute, board policy or
agreement) separately from `source_url`, because a citation like
`Wis. Stat. § 36.31(2m)(b)` is a better and more stable source than most campus
explainer pages, and several of these rules have no canonical URL at all. Every
row remains `needs_check`: the research grade says how far each is from
promotion, not that it is promoted.

## The all-states audit (2026-09-20)

`npm run audit` compares the shipped dataset against `data/research/` and exits
non-zero on drift. A generated file that has silently stopped matching its
source is the same wrong-but-plausible data this project refuses, so the check
is a program rather than a memory.

Its first run found 5 drift items — all three tier-1 states were missing the
governing authority the statewide rows carry, and Florida's and Texas's
guarantee text no longer named their own framework. Fixed. It now reports
**0 drift, 1 gap** (Wisconsin's promise, which the research itself marks
unconfirmed).

## Aid, and the two kinds that may be subtracted

39 states now carry a named fee-waiver or promise programme and 50 carry a
dual-enrolment programme. **Only 12 may be applied to a price**: a need-tested
waiver of the fee itself, or a promise that is genuinely universal.

The other 27 are gated on age, graduation year, field of study or service —
things this app never asks about. Subtracting one would quote a student a price
they may never be offered, which is wrong in their favour and therefore never
reported. `AidKind` carries the gate, `effectiveCost` consults only the two safe
kinds, and everything else is surfaced as an opportunity with the prices left
alone. A compound kind reads as its **most gated half**.

Dual enrolment is never subtracted at all, however universal — Florida's is
fee-exempt by statute and still does not discount the plan, because it is a
different and cheaper way to earn the same credit rather than a discount on
buying it the expensive way.

## Third-party credit: six stances, not two

`Jurisdiction.third_party` replaces a silence. `refuses` on an Institution
expressed a published refusal and nothing else, so a state where the **law**
requires an evaluation on request looked identical to one where nobody has ever
said anything. Florida is the former (Fla. Stat. § 1004.0961 — you have a right
to be evaluated, on request, before your first term). Most states are the
latter.

No state found lets third-party credit into a statewide general-education block
by right. The eight public destinations that appear on three or more provider
partner lists — Charter Oak, Thomas Edison, SUNY Empire, UMGC, UMPI, CSU Global,
Purdue Global, UAGC — are the realistic targets, and `docs/OUTCOMES.md` says so.

## What is missing, per state

`data/GAPS.md`, generated by `npm run gaps:write`. It reads the same rows the
app reads, so it cannot go stale the way a hand-written list does. It also
carries the gaps a data audit cannot detect — the ones where the FIELD does not
exist, so no amount of research helps until the type changes. The worst of those
is that every unit here is assumed to be a semester credit and nothing records
the assumption; Washington and Oregon count in quarters.
