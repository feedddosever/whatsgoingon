import type {
  ...
  PlanItem, Route, RouteKind, StudentInput,
} from './types.ts';
...
export function planRoute(ds: Dataset, input: StudentInput, kind: RouteKind): Route {
  ...
  if (kind === 'lowest_risk' && items.some(i => i.provenance.confidence === 'unverified')) {
    warnings.push('Lowest-risk route still contains unverified rows — dataset is not ready to ship.');
  }
